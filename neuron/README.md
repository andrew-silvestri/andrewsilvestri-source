...
python3 build_neuron.py          # the census, the trade-off, the gradient test, the opened set -> outputs/neuron_payload.json
python3 build_neuron.py --build  # -> neuron-app.html
python3 fig_neuron.py            # five figures -> ../site/assets/neuron_fig*.png, each audited (must print 0 problems)
...
| `build_neuron.py` | The cascade, all 32 subsets, the trade-off measured both from metadata and from files, the within-archive gradient test, the span arithmetic, the complete set as its files hold it, and the geometry of the two drawing figures (the panel boxes, and what a pixel is in each). Writes `outputs/neuron_payload.json`. With `--build`, also the app. |
| `build_app.py` | Writes `neuron-app.html` from `template-app.html` and ~1.2 KB of subset counts. |
| `fig_neuron.py` | The five figures, through `sitefig.py`. No figure uses a z coordinate. Figures 2 and 5 draw widths as polygons in data units, so a width is the file's number at the panel's scale; a file holding one width is drawn as a hairline. |
| `update_page.py` | Writes `../site/neuron.html` from `template.html` and the payload. |
| `test_neuron.py` | Eight failure modes, listed in its docstring. |
