...
(function () {
  ...
  function draw() {
    ...
    /* 66, not 30: the foot (the notes line and the hint, two lines of 12px
       with padding) sits over the bottom of this canvas, and at 30 the
       meter's bars ran under "sounding G3 B3 D4..." (2026-09-04). The
       test's label check reads this value, so lowering it fails loudly. */
    var eqBase = H - 66 * dpr;
    var mx = peak();
    ...
    /* every piece of text drawn on the canvas goes in here as a box, so a
       label placed later can be tested against all of them, compass points
       included - "One World Trade Center" printed straight through "NW"
       until they were (2026-09-04) */
    var placed = [];
    var lineH = 12 * dpr;
    for (var d2 = 0; d2 < 360; d2 += 45) {
      ...
      var cxp = cx + Rx * 1.10 * Math.sin(ph2);
      var cyp = cyc + Ry * 1.10 * Math.cos(ph2) + 20 * dpr;
      ctx.fillText(compass(d2), cxp, cyp);
      placed.push({ x: cxp, half: ctx.measureText(compass(d2)).width / 2, y: cyp });
    }
    ctx.globalAlpha = 1;
    ...
    var gap = 14 * dpr;
    var named = 0;
    // the names sit a line below the compass ring, not on it
    var ty = splitY + 34 * dpr;
    ...
    for (i = 0; i < seen.length && named < maxLabels; i++) {
      var nm = seen[i][4];
      ...
      for (var j = 0; j < placed.length; j++) {
        // boxes overlap: within a line of each other vertically and
        // closer than their half-widths plus the gap horizontally
        if (Math.abs(ty - placed[j].y) < lineH + 2 * dpr &&
            Math.abs(tx - placed[j].x) < half + placed[j].half + gap) {
          clash = true; break;
        }
      }
      if (clash) continue;
      placed.push({ x: tx, half: half, y: ty });
      named++;
      ctx.fillStyle = dim;
